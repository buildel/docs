...
<video controls width="100%" autoPlay loop>
  <source type="video/webm" src="/mdx-assets/guides/create-first-workflow/run-simple-workflow.webm"/>
</video>
...
Find the [chat](/docs/blocks/chat.mdx) block in the sidebar and add it between the two blocks you've already placed. You might notice the chat block has more input/output types, but we'll delve into that later.
...
<video controls width="100%" autoPlay loop>
  <source type="video/webm" src="/mdx-assets/guides/create-first-workflow/configure-llm.webm"/>
</video>
...
In Buildel, you can easily switch between LLM models from different providers. The only requirement is to provide the API Key obtained from the chosen provider. We talk more about it [here](/docs/secrets).
...
<video controls width="100%" autoPlay loop>
  <source type="video/webm" src="/mdx-assets/guides/create-first-workflow/run-llm-workflow.webm"/>
</video>

Cool, right? With just a few clicks, you've created your first fully working AI assistant that can now be [used](/docs/interfaces.mdx) in your applications. I encourage you to play around with the configuration and see how the chat functions.

For more useful information about the chat block, check out [here](/docs/blocks/chat.mdx).
